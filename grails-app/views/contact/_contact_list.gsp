				...
				<li class="${c == contactInstance ? 'selected' : ''}" id="contact-${c.id}">
					<g:if test="${contactsSection instanceof frontlinesms2.Group}">
						<g:link class="displayName-${c.id}" controller="contact" action="show" params="[contactId:c.id, groupId:contactsSection.id]">
							<g:checkBox name="contact" checked="${params.checkedId == c.id+'' ? 'true': 'false'}" value="${c.id}" onclick="updateContactDetails(${c.id});"/>	${c.name?:c.primaryMobile?:c.secondaryMobile?:'[No Name]'}</g:link>
					</g:if>
					<g:else>
