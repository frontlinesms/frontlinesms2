			...
			<g:each in="${contactInstanceList}" status="i" var="c">
				<li class="${c.id == contactInstance?.id ? 'selected' : ''}" id="contact-${c.id}">
					...
					<g:if test="${contactsSection instanceof frontlinesms2.Group}">
						<g:link class="displayName-${c.id} contact-name" controller="contact" action="show" params="[contactId:c.id, groupId:contactsSection.id, sort: params.sort, offset: params.offset]">
							${c.name?:c.primaryMobile?:c.secondaryMobile?:'[No Name]'}
						</g:link>
					</g:if>
					<g:else>
						<g:link class="displayName-${c.id}" action="show" params="[contactId: c.id, sort: params.sort, offset: params.offset]">
							${c.name?:c.primaryMobile?:c.secondaryMobile?:'[No Name]'}
						</g:link>
					</g:else>
				</li>
