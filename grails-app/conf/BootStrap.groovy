...
class BootStrap {
	...
	private def dev_initFmessages() {
		...
		[new Fmessage(src:'+123456789', dst:'+2541234567', text:'manchester rules!', date:new Date()),
				new Fmessage(src:'+198765432', dst:'+254987654', text:'go manchester', date:new Date()),
				new Fmessage(src:'Joe', dst:'+254112233', text:'pantene is the best', date:new Date()-1),
				new Fmessage(src:'Jill', dst:'+254987654', text:"where's the hill?", date:createDate("2011/01/21")),
				new Fmessage(src:'+254675334', dst:'+254112233', text:"where's the pale?", date:createDate("2011/01/20")),
				new Fmessage(src:'Humpty', dst:'+254112233', text:"where're the king's men?", starred:true, date:createDate("2011/01/23"))].each() {
			it.inbound = true
			it.save(failOnError:true)
		}
		println "message date: ${Fmessage.findBySrc('Jill').date}"
		println "shpuld be: ${createDate('2011/01/21')}"
		
		(1..101).each {
			new Fmessage(src:'+198765432', dst:'+254987654', text:"text-${it}", date: new Date() - it, inbound:true).save(failOnError:true)
		}
	...
	private def dev_initPolls() {
		...
		10.times {
			def msg = new Fmessage(src: "+9198765432${it}", dst: "+4498765432${it}",date: new Date() - it, text: "Yes", inbound:true);
	...
	private def dev_initRadioShows() {
		...
		r.addToMessages(new Fmessage(text: "eat fruits", src: "src", dst: "dst", date: new Date()))
		r.addToMessages(new Fmessage(text: "excerise", src: "src", dst: "dst", date: new Date()))
