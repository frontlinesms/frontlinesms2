...
class MessageActionSpec extends frontlinesms2.poll.PollGebSpec {
	...
	def 'clicking on poll moves the message to that poll and removes it from the previous poll or inbox'() {
		...
		then:
			bob != Poll.findByTitle("Football Teams").getMessages(false, null, null).find { it == bob }
			bob == Poll.findByTitle("Shampoo Brands").getMessages(false, null, null).find { it == bob }
		...
		then:
			jill != Fmessage.getInboxMessages(false, null, null).find { it == jill }
			jill == Poll.findByTitle("Football Teams").getMessages(false, null, null).find { it == jill }
		cleanup:
			deleteTestPolls()
			deleteTestMessages()
	}
	...
	def 'clicking on folder moves the message to that folder and removes it from the previous location'() {
		...
		when:
			...
			workFolder.refresh()
		then:
			max != footballPoll.getMessages(false, null, null).find { it == max }
			max == workFolder.getFolderMessages(false, null, null).find { it == max }
		cleanup:
			deleteTestFolders()
			deleteTestPolls()			
			deleteTestMessages()
