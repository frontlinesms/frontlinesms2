...
class QuickMessageController {
	def create = {
		...
		def recipientName = recipients.size() == 1 ? (Contact.findByPrimaryMobile(recipients[0])?.name ?: recipients[0]) : ""
		...
		[contactList: contacts,
				...
				nonExistingRecipients:recipients - contacts*.getPrimaryMobile() - contacts*.getEmail()]
	}
