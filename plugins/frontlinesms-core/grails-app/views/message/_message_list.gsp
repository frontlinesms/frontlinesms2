		...
		<g:if test="${messageInstanceTotal > 0}">
			...
			<g:else>
				...
				<g:each in="${messageInstanceList}" status="i" var="m">
					<tr class="message-preview ${m == messageInstance ? 'selected initial-selection' : ''} ${m.read?'read':'unread'} ${m.archived?'archived':''} ${m.hasSent? 'sent':''} ${m.hasPending? 'pending':''} ${m.hasFailed? 'failed':''} ${m.ownerDetail? m.ownerDetail.toLowerCase() :''}" id="message-${m.id}">
