<div class="generic_sorting_tab">
	...
	<div class="input">
		...
		<g:textField name="keywords" class="${!activityInstanceToEdit || activityInstanceToEdit?.keywords ? 'required sorting-generic' : ''}" value="${activityInstanceToEdit?.keywords? activityInstanceToEdit?.keywords*.value.join(',') : ''}" disabled="${activityInstanceToEdit && !activityInstanceToEdit?.keywords}"/>
	</div>
