...
<div class="input">
	<g:textField name="topLevelKeywords" value="${activityInstanceToEdit?.keywords?.findAll { it.isTopLevel && !it.ownerDetail}?.value?.join(',') }" class="sorting-generic subscription-keyword-field"/>
</div>
...
<div class="input">
	<table class="subscription-aliases">
		<tr>
			...
			<td><g:textField class="keywords validcommas subscription-keyword-field" name="joinKeywords" id="joinKeywords" value="${activityInstanceToEdit?.keywords?.findAll { it.ownerDetail == 'JOIN' }?.value?.join(',') }"/></td>
		</tr>
		<tr>
			<td><label for="leaveKeywords"><g:message code="subscription.keywords.leave"/></label></td>
			<td><g:textField class="keywords validcommas subscription-keyword-field" name="leaveKeywords" id="leaveKeywords" value="${activityInstanceToEdit?.keywords?.findAll { it.ownerDetail == 'LEAVE' }?.value?.join(',') }"/></td>
		</tr>
	</table>
</div>
