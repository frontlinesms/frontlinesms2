			...
			<li>
				<g:if test="${activityInstanceToEdit?.id}">
					...
					<g:if test="${(alias == 'A' || alias == 'B' || pollResponse?.value || (i == (activityInstanceToEdit?.responses.size() - 2)))}">
						<g:textField class='aliases required validcommas' name="alias${alias}" value="${pollResponse?.aliases}"/>
					</g:if>
					<g:else>
						<g:textField class='aliases required validcommas' name="alias${alias}" value="${pollResponse?.aliases}" disabled="true"/>
					</g:else>
				</g:if>
				<g:else>
					<label for='alias${alias}' class="${alias == 'A' || alias == 'B' ? 'field-enabled': ''}">${option}</label>
					<g:if test="${(alias == 'A' || alias == 'B')}">
						<g:textField class='aliases required validcommas' name="alias${alias}"/>
					</g:if>
					<g:else>
						<g:textField class='aliases required validcommas' name="alias${alias}" disabled="true"/>
					</g:else>
				</g:else>
			</li>
		</g:each>
