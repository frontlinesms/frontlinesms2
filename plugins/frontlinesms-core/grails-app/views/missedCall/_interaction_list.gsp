...
<g:hiddenField name="messageTotal" value="${missedCallInstanceTotal}"/>
...
<table id="main-list" class="no-content-column">
	...
	<tbody>
		<g:if test="${missedCallInstanceTotal > 0}">
			<g:if test="${messageSection == 'trash' && !params.starred}">
				<fsms:render template="/interaction/trash_list"/>
			</g:if>
			<g:else>

				<g:each in="${missedCallInstanceList}" status="i" var="m">
					<tr class="message-preview ${m == missedCallInstance ? 'selected initial-selection' : ''} ${m.read?'read':'unread'} ${m.archived?'archived':''}" id="message-${m.id}">
