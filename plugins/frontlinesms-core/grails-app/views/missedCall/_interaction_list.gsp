...
<table id="main-list" class="no-content-column">
	<thead>
		<tr>
			<th>
				<fsms:checkBox name="interaction-select" class="interaction-select" id="interaction-select-all" value="0" checked="false" onclick="check_list.checkAll('interaction', 'missedCall')" disabled="${messageSection == 'trash'}"/>
			</th>
			<th></th>
			...
			<g:else>
				...
				<g:each in="${interactionInstanceList}" status="i" var="m">
					<tr class="interaction-preview ${m == interactionInstance ? 'selected initial-selection' : ''} ${m.read?'read':'unread'} ${m.archived?'archived':''}" id="interaction-${m.id}">
						...
						<td id="star-${m.id}" >
							<g:remoteLink class="${m.starred ? 'starred' : 'unstarred'}" controller="missedCall" action="changeStarStatus" params='[interactionId: "${m.id}"]' onSuccess="setStarStatus('star-${m.id}', data)"/>
						</td>
