...
class FsmsTagLib {
	...
	def submenu = { att, body ->
		def icon
		def iconMap = ['messages': 'envelope', 'missedCalls': 'phone', 'activities': 'comments', 'folders':'folder-open', 'contacts':'user', 'groups':'users', 'smartgroups':'cog']
		...
		if(att.code) {
			out << '<h3>'
			if(icon) {
				out << "<i class='menuIcon fa fa-$icon'></i>"
			}
