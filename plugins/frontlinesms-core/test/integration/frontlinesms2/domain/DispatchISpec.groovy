...
class DispatchISpec extends grails.plugin.spock.IntegrationSpec {
	def "updating dispatch status to SENT should update the associated message status"() {
		setup:
			...
			def message = new TextMessage(text:"test", inbound:false, date:new Date()).addToDispatches(dis).save(flush:true, failOnError:true)
			message = TextMessage.get(message.id)
