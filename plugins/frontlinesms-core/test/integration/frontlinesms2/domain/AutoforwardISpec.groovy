...
class AutoforwardISpec extends grails.plugin.spock.IntegrationSpec {
	def "the outgoing message created by processKeyword should have the owner detail set to the id of the triggering incoming message"() {
		when:
			...
			def desiredOutbound = new TextMessage(inbound:false, text:"Someone said something").addToDispatches(new Dispatch(dst:"12345", status:DispatchStatus.PENDING)).save(failOnError:true)
			messageSendService.createOutgoingMessage(_) >> desiredOutbound
			def inbound = new TextMessage(inbound: true, src:"54321", text:"this should trigger an outgoing message").save()
			...
			def outbound = TextMessage.findByText('Someone said something')
		then:
